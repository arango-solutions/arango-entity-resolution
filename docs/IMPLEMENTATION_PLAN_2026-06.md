...
### 1.4 Schema-agnostic defaults (M) — ✅ DONE

**Status:** `FieldProfiler` implemented. Pure `classify_values` detects field type (email/phone/date/numeric/id/person-name/org-name/address/free-text/short-string) from regex + statistics (cardinality, length, token counts); `TYPE_DEFAULTS` maps each type to a comparator, transformer chain, agreement threshold, and seed m/u priors. `profile()` samples the collection and reports per-field type + completeness; `emit_similarity_config()` produces a ready similarity block (normalized weights, transformers, thresholds, EM seed priors), skipping near-empty and free-text fields. `arango-er profile --collection X [--emit-config]` CLI. Classification fully unit-tested. **Integration test written (`tests/test_field_profiler_integration.py`) but not yet executed — Docker was unavailable at commit time; the sampling AQL is the same `SORT RAND() LIMIT` pattern already validated in the 1.1A estimator integration test.** Together with EM (1.1), this removes the hand-written weights dict: profiling picks comparators + seed priors, EM learns weights.
